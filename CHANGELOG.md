...
### Added
- Opt-in **voice follow**: the prompter scrolls while you speak and pauses
  when you stop, using a local Web Audio voice-activity detector (no audio
  recorded or sent anywhere). Toggle it in the control window's Voice
  section, the prompter HUD mic button, or the `V` shortcut; a mic
  sensitivity slider tunes speech detection.
